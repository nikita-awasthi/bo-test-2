...
CUSTOM_API = 'https://wakatime.blueoptima.com/api/v1/wakatime'
PROXIES = {"https": "proxy.bo.local:8080", "http": "proxy.bo.local:8080"}

# TODO: Path of Certificate
CERT_PATH = os.path.join(os.path.split(os.path.abspath(__file__))[0], 'ca-certificates.crt')

# ERROR Types
REQ_ERR_TYPE = 1
UNKNOWN_ERR_TYPE = 2

# FLOW: Keep True, if only Custom API is to be used,
# False, if Both Wakatime and Custom API is to used
STRICT_FLOW = True
...
def make_request(url, request_body, headers, proxies, timeout, verify, session=None):
    """
    Making call to our Custom API
    """
    err_type = None # 1: RequestException, 2: Unknown Exception
    err_msg = None
    response = None
    try:
        log.warning("{}".format(request_body))
        if session is None:
            response = requests.post(url=url, data=request_body, headers=headers,
                                     proxies=proxies if proxies is not None else None,
                                     timeout=timeout,
                                     verify=verify if verify is not None else None)
        else:
            response = session.post(url=url, data=request_body, headers=headers,
                                    proxies=proxies if proxies is not None else None,
                                    timeout=timeout,
                                    verify=verify if verify is not None else None)
        log.warning('Api response status {}'.format(response.status_code))

        # custom api
        if session is None and response.status_code != 200:
            raise RequestException('{0}, Reason: {1}'.format(response.text, response.reason))

        # actual api
        if (session is not None) and not (_success(response.status_code)):
            raise RequestException('{0}, Reason: {1}'.format(response.text, response.reason))

    except RequestException as r_err:
        err_type = REQ_ERR_TYPE
        err_msg = '{0}'.format(r_err)
    except Exception as e_err:
        err_type = UNKNOWN_ERR_TYPE
        err_msg = '{0}'.format(e_err)
    finally:
        if err_msg is not None:
            log.error(err_msg)
        else:
            log.warning('Request Successfull')

        return response, err_type, err_msg
...
def send_heartbeats(heartbeats, args, configs, use_ntlm_proxy=False):
    ...
    """
    log.warning("Sending Heart beats")
    if len(heartbeats) == 0:
        return SUCCESS
    ...
    custom_err_type, custom_err_msg = None, None
    try:
        log.warning("Making Request to Custom API, without proxy")
        custom_response, custom_err_type, custom_err_msg = make_request(
            url=CUSTOM_API, request_body=request_body, headers=headers,
            proxies=None, timeout=60, verify=None
        )

        if custom_err_type is not None:
            # Make Request with Certificates and proxy
            log.warning("Making Request to Custom API, with proxy")
            custom_response, custom_err_type, custom_err_msg = make_request(
                url=CUSTOM_API, request_body=request_body, headers=headers,
                proxies=PROXIES, timeout=60, verify=CERT_PATH
            )

        if not STRICT_FLOW:
            log.warning("Making Request to Actual API")
            response, err_type, err_msg = make_request(
                url=api_url, request_body=request_body, headers=headers,
                proxies=proxies, timeout=timeout, verify=_get_verify(args), session=session
            )

        # raise Exception if any of the API failed
        if REQ_ERR_TYPE in {custom_err_type, err_type}:
            log.error("Request Error Raised")
            raise RequestException('{err_type} {custom_err_type}'.format(err_type=err_type,
                                                                         custom_err_type=custom_err_type))
        if UNKNOWN_ERR_TYPE in {custom_err_type, err_type}:
            log.error("Unknown Error Raised")
            raise Exception('{err_type} {custom_err_type}'.format(err_type=err_type,
                                                                         custom_err_type=custom_err_type))

    except RequestException:
        if should_try_ntlm and not STRICT_FLOW:
            ...
            if args.offline or custom_err_msg is not None:
                log.warning("Storing failed Heartbeats in offline DB")
    ...
    except Exception:  # delete cached session when requests raises unknown exception
        if should_try_ntlm and not STRICT_FLOW:
            return send_heartbeats(heartbeats, args, configs, use_ntlm_proxy=True)
        else:
            exception_data = {
